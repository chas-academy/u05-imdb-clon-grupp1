        ...
        <div class="focus:outline-none h-80 w-screen my-4 max-w-screen-2xl flex overflow-x-scroll container select-none ml-4">
            <div v-for="(movie, index) in moviesPrint" :key="index" class="relative">
                <div class="m-2 h-72 w-44 rounded-3xl relative ">
                    <img :src="movie.img_path" :alt="movie.title" class="h-full w-full rounded-3xl cursor-pointer object-cover shadow-md border-r border-gray-700 border-opacity-30" >
                    <div @mousedown="mouseStart(movie.id)" class="gradient absolute w-full h-full top-0 rounded-2xl opacity-100 lg:opacity-0 lg:hover:opacity-100 transition-opacity duration-500" draggable="false">
    ...
    export default {
        ...
        methods: {
            mouseStart(id) {
                ...
                let e = event.target.parentElement.parentElement.parentElement;

                e.onmousedown = (e) => isMouseDown = true;
